...
bool wxGenericValidator::TransferToWindow()
{
    ...
    if (wxDynamicCast(m_validatorWindow, wxButton))
    {
        ...
        {
            pControl->SetLabel(m_pStdString->c_str()) ;
            return true;
        }
    } else
#endif
    ...
    if (wxDynamicCast(m_validatorWindow, wxComboBox))
    {
        ...
        {
            if (pControl->FindString(m_pStdString->c_str()) != wxNOT_FOUND)
            {
                pControl->SetStringSelection(m_pStdString->c_str());
            }
            if ((m_validatorWindow->GetWindowStyle() & wxCB_READONLY) == 0)
            {
                pControl->SetValue(m_pStdString->c_str());
            }
            return true;
        }
    } else
...
bool wxGenericValidator::TransferFromWindow()
{
    ...
    if (wxDynamicCast(m_validatorWindow, wxButton))
    {
        ...
        if (m_pStdString)
        {
            *m_pStdString = pControl->GetLabel().ToStdString() ;
            return true;
        }
    } else
#endif
    ...
    if (wxDynamicCast(m_validatorWindow, wxComboBox))
    {
        ...
        {
            if (m_validatorWindow->GetWindowStyle() & wxCB_READONLY)
                *m_pStdString = pControl->GetStringSelection().ToStdString();
            else
                *m_pStdString = pControl->GetValue().ToStdString();
            return true;
        }
    } else
#endif
