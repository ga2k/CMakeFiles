...
function(record)
    ...
    elseif(_recVerbUC STREQUAL "DUMP")
        ...
        set(_dumpStr "record '${_recVar}' = [\n")
        set(_i 0)
        foreach(_f IN LISTS _lst)
            _hs__field_to_user("${_f}" _v)
            string(APPEND _dumpStr "  [${_i}] = \"${_v}\"\n")
            math(EXPR _i "${_i} + 1")
        endforeach()
        string(APPEND _dumpStr "]")

        if(${ARGC} EQUAL 3)
            set(${ARGV2} "${_dumpStr}" PARENT_SCOPE)
        else()
            message("${_dumpStr}")
        endif()
        return()
...
function(array)
    ...
    if(_V STREQUAL "DUMP")
        ...
        set(_dumpStr "array '${arrayVarName}' (kind=${_kind}, len=")
        list(LENGTH _lst _len)
        string(APPEND _dumpStr "${_len}) = [\n")

        set(_i 0)
        foreach(_elem IN LISTS _lst)
            string(APPEND _dumpStr "  [${_i}] = \"${_elem}\"\n")
            math(EXPR _i "${_i} + 1")
        endforeach()
        string(APPEND _dumpStr "]")
