...
function(fetchContents)
    ...
    while (aix LESS numPackages)
        ...
        # Step 1. Merge Args
        list(GET _uPackage ${FIXArgs} _uArgs)
        list(GET _sPackage ${FIXArgs} _sArgs)


        if (NOT "${_uArgs}" STREQUAL "${_sArgs}")
            string(REPLACE "${_TOK_LIST_SEP}" ";" _uArgs "${_uArgs}")
            string(REPLACE "${_TOK_LIST_SEP}" ";" _sArgs "${_sArgs}")
            if (REQUIRED IN_LIST _sArgs AND OPTIONAL IN_LIST _uArgs)
                list(REMOVE_ITEM _uArgs OPTIONAL)
            elseif (REQUIRED IN_LIST _uArgs AND OPTIONAL IN_LIST _sArgs)
                list(REMOVE_ITEM _sArgs OPTIONAL)
            endif ()
            list(APPEND _sArgs ${_uArgs})
        ...
        list(GET _uPackage ${FIXComponents} _uComps)
        list(GET _sPackage ${FIXComponents} _sComps)
        if (NOT "${_uComps}" STREQUAL "${_sComps}")
            string(REPLACE "${_TOK_LIST_SEP}" ";" _uComps "${_uComps}")
            string(REPLACE "${_TOK_LIST_SEP}" ";" _sComps "${_sComps}")
    ...
    function(processFeatures features feature_names)
        ...
        math(EXPR usableCols "${_term_cols} - 16")
        ...
        foreach (pass_num RANGE 1)
            ...
            set(phase ${pass_num})
            inc(phase)
            msg("\n${BOLD}${GREEN}Phase ${phase} ${BOLD}${INVGREEN}${line}${NC}\n")
