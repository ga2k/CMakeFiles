...
function(fetchContents)
    ...
    collection(CREATE USER_FEATURES)
    ...
    if(numPackages)
        foreach(pkgIndex RANGE ${numPackages})
            ...
            record(GET requestedFeature ${FIXName}        featureName)    # POP_FRONT on a FEATURE turns it into a PACKAGE
            record(GET requestedFeature ${FIXPkgName} packageName)
            ...
            elseif (${featureName} IN_LIST SYSTEM_NAMES) # OR ${featureName} IN_LIST LIBRARY_NAMES)
                ...
                if (NOT potentialFeature OR "${potentialFeature}" STREQUAL "" OR "${potentialFeature}" STREQUAL "NOTFOUND")
    ...
    function(processFeatures features featureList)
        ...
        foreach(k IN LISTS featureList)
            ...
            record(GET k ${FIXName} f)
            record(GET k ${FIXPkgName} n)
            math(EXPR fix "${fix} + 1")
            SplitAt("${c}" "" x g)
            if ("${g}" STREQUAL "*")
