...
function(fetchContents)
    ...
    object (CREATE hCUSTOM   KIND DICT LABEL CUSTOM)
    object (CREATE hLIBRARY  KIND DICT LABEL LIBRARY)
    object (CREATE hOPTIONAL KIND DICT LABEL OPTIONAL)
    object (CREATE hPLUGIN   KIND DICT LABEL PLUGIN)
    object (CREATE hSYSTEM   KIND DICT LABEL SYSTEM)

    object (CREATE hCustomNames      KIND RECORD LABEL CUSTOM_FEATURES)
    object (CREATE hCustomPackages   KIND RECORD LABEL CUSTOM_PACKAGES)
    object (CREATE hLibraryNames     KIND RECORD LABEL LIBRARY_FEATURES)
    object (CREATE hLibraryPackages  KIND RECORD LABEL LIBRARY_PACKAGES)
    object (CREATE hOptionalNames    KIND RECORD LABEL OPTIONAL_FEATURES)
    object (CREATE hOptionalPackages KIND RECORD LABEL OPTIONAL_PACKAGES)
    object (CREATE hPluginNames      KIND RECORD LABEL PLUGIN_FEATURES)
    object (CREATE hPluginPackages   KIND RECORD LABEL PLUGIN_PACKAGES)
    object (CREATE hSystemNames      KIND RECORD LABEL SYSTEM_FEATURES)
    object (CREATE hSystemPackages   KIND RECORD LABEL SYSTEM_PACKAGES)
#
#    object (SET hCUSTOM   NAME EQUAL NAMES    HANDLE hCustomNames)
#    object (SET hCUSTOM   NAME EQUAL PACKAGES HANDLE hCustomPackages)
#    object (SET hLIBRARY  NAME EQUAL NAMES    HANDLE hLibraryNames)
#    object (SET hLIBRARY  NAME EQUAL PACKAGES HANDLE hLibraryPackages)
#    object (SET hOPTIONAL NAME EQUAL NAMES    HANDLE hOptionalNames)
#    object (SET hOPTIONAL NAME EQUAL PACKAGES HANDLE hOptionalPackages)
#    object (SET hPLUGIN   NAME EQUAL NAMES    HANDLE hPluginNames)
#    object (SET hPLUGIN   NAME EQUAL PACKAGES HANDLE hPluginPackages)
#    object (SET hSYSTEM   NAME EQUAL NAMES    HANDLE hSystemNames)
#    object (SET hSYSTEM   NAME EQUAL PACKAGES HANDLE hSystemPackages)
#
#    object (DUMP hCUSTOM)
#    object (DUMP hCUSTOM VERBOSE)
#    object (DUMP hLIBRARY)
#    object (DUMP hOPTIONAL)
#    object (DUMP hPLUGIN)
#    object (DUMP hSYSTEM)
...
#    endforeach ()
    ...
    object(CREATE_VIEW hGlobal LABEL DATA FROM hSYSTEM hLIBRARY hOPTIONAL hPLUGIN hCUSTOM)
    dict(DUMP DATA)
    ...
    while(aix LESS numPackages)
        ...
        # There are no SYSTEM default packages in the users list...

        if (packageName)
            getFeaturePackageByName(DATA ${featureName} ${packageName} potentialFeature index)
        endif ()
        ...
        if(NOT "${_userBits}" STREQUAL "${_regdBits}")
            ...
            record(REPLACE wip ${FIXArgs} "${_bits}")
        endif ()
    ...
    dict(GET SYSTEM "NAMES" featureNames)
    ...
    while(featureIndex LESS numFeatureNames)
        ...
        dict(GET SYSTEM "EQUAL" "${thisFeatureName}" thisFeature)
