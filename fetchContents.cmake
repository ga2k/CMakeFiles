...
function(fetchContents)
    ...
    collection(CREATE CUSTOM)
    collection(CREATE GLOBAL)
    collection(CREATE LIBRARY)
    collection(CREATE OPTIONAL)
    collection(CREATE PLUGIN)
    collection(CREATE SYSTEM)

    record(CREATE customNames       NAMES)
    record(CREATE customPackages    PACKAGES)
    record(CREATE globalNames       NAMES)
    record(CREATE globalPackages    PACKAGES)
    ...
    record(CREATE pluginNames       NAMES)
    record(CREATE pluginPackages    PACKAGES)
    record(CREATE systemNames       NAMES)
    record(CREATE systemPackages    PACKAGES)

    collection(SET CUSTOM   NAMES    "${customNames}")
    collection(SET CUSTOM   PACKAGES "${customPackages}")
    collection(SET GLOBAL   NAMES    "${globalNames}")
    collection(SET GLOBAL   PACKAGES "${globalPackages}")
    collection(SET LIBRARY  NAMES    "${libraryNames}")
    collection(SET LIBRARY  PACKAGES "${libraryPackages}")
    collection(SET OPTIONAL NAMES    "${optionalNames}")
    collection(SET OPTIONAL PACKAGES "${optionalPackages}")
    collection(SET PLUGIN   NAMES    "${pluginsystemNames}")
    collection(SET PLUGIN   PACKAGES "${pluginsystemPackages}")
    collection(SET SYSTEM   NAMES    "${systemNames}")
    collection(SET SYSTEM   PACKAGES "${systemPackages}")
#
    set(options HELP)
    ...
    foreach(DRY_RUN IN ITEMS ON OFF)
        ...
        set(gmloptryiporuet_IX 0)
        record(DUMP __regPkgCB VERBOSE)
        set(__regPkgCB $CACHE{__regPkgCB})
        record(DUMP __regPkgCB VERBOSE)
        record(LENGTH __regPkgCB gmloptryiporuet_MAX)
        while(gmloptryiporuet_IX LESS gmloptryiporuet_MAX)
            set(totpIX ${gmloptryiporuet_IX})
            inc(gmloptryiporuet_IX)
            record(GET __regPkgCB ${totpIX} fn)
    ...
    while(aix LESS numPackages)
        ...
        if (packageName)
            getFeaturePackageByName(GLOBAL ${featureName} ${packageName} potentialFeature index)
        endif ()
    ...
    collection(GET SYSTEM "NAMES" featureNames)
    ...
    while(featureIndex LESS numFeatureNames)
        ...
        collection(GET SYSTEM "EQUAL" "${thisFeatureName}" thisFeature)
                            ...
                            if (${this_pkgname}_FOUND)
                                ...
                                if (this_prereqs)
                                    ...
                                    foreach(p IN LISTS this_prereqs)
                                        ...
                                        if(prePkg)
                                            getFeaturePackageByName("${GLOBAL}" "${preFeatName}" "${prePkgName}" prePkg dc)
                                        else ()
                                            getFeaturePackageBy("${GLOBAL}" "${preFeatName}" 0  prePkg)
                                        endif ()
