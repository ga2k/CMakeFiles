...
#    set(argn ${ARGN})
#    if (argn MATCHES "^(.*)ALWAYS(.*)$")
#        set(AA_ALWAYS ON)
#        set(argn "${CMAKE_MATCH_1}${CMAKE_MATCH_2}")
#    endif ()
#    if (argn MATCHES "^(.*)VERBATIM(.*)$")
#        set(AA_VERBATIM ON)
#        set(argn "${CMAKE_MATCH_1}${CMAKE_MATCH_2}")
#    endif ()
...
function(doDump)
    ...
    macro(mmmChocolate words)
        msg(${mode} ${words} VERBATIM)
    endmacro()

    macro(mmmmChocolate words)
        if (AA_FATAL_ERROR)
        ...
        elseif (AA_CHECK_START)
            m(AA_CHECK_START "${words}" VERBATIM)
        elseif (AA_CHECK_PASS)
            m(CHECK_PASS "${words}" VERBATIM)
        elseif (AA_CHECK_FAIL)
            m(CHECK_FAIL "${words}" VERBATIM)
        else ()
...
function(replaceFiles target patchList)
    ...
    foreach (patch IN LISTS patchList)
        ...
        foreach (file_rel_path IN LISTS override_files)
            ...
            if (errored)
                msg(CHECK_FAIL "${RED}[FAILfstjtdfghfdED]${NC} ${BOLD}${error_msg}${NC}")
                set(any_errored ON)
                set(errored OFF)
                set(error_msg)
            else ()
...
function(columnarTextOut ID FIELD0 FIELD1 FIELD2 FIELD3 FIELD4 FIELD5 TEMPLATE DRY_RUN)
    ...
    foreach (item ${placeholders})
        if (item MATCHES "\\[([_A-Z0-9]+):([LCR])\\]")
            ...
            set(unpadded "${${current_tag}}")
            set(SELECT_OK OFF)
            SELECT(longest AS _longest FROM tbl_LongestStrings WHERE ROW = "${rowName}")
            longest(${JUSTIFY} CURRENT ${_longest} TEXT "${unpadded}" PADDED padded LONGEST __longest)
            set("${current_tag}" "${padded}")
