...
set(_TOK_NOUN_ROW    "α")   # No longer replaces "ROW"    in WHERE
set(_TOK_NOUN_COLUMN "β")   # No longer replaces "COLUMN" in WHERE
set(_TOK_NOUN_ROWID  "δ")   # No longer replaces "ROWID"  in WHERE
set(_TOK_NOUN_INDEX  "ε")   # No longer replaces "INDEX"  in WHERE
set(_TOK_NOUN_NAME   "θ")   # No longer replaces "NAME"   in WHERE
set(_TOK_NOUN_KEY    "μ")   # No longer replaces "KEY"    in WHERE
set(_TOK_FS "␟")
set(_TOK_RS "␞")
set(_TOK_GS "␝")
...
# --------------------------------------------------------------------------------------------------
globalObjSet (HS_NEXT_HNDL 1000)

function(_hs_sql_internal_insert hndl values)
    globalObjGet("${hndl}_COLUMNS" _encCols)
    _hs_sql_record_to_list(_encCols _cols)
    globalObjGet("${hndl}_NEXT_ROWID" _nextID)
    ...
    foreach (_c IN LISTS _cols)
        ...
        globalObjSet ("${hndl}_R${_nextID}_${_c}" "${_v}")
        math(EXPR _vIdx "${_vIdx} + 1")
    endforeach ()

    globalObjAppendUnique("${_nextID}" "${hndl}_ROWIDS")
    globalObjGet("${hndl}_ROW_COUNT" _count)
    math(EXPR _newCount "${_count} + 1")
    globalObjSet ("${hndl}_ROW_COUNT" "${_newCount}")
    math(EXPR _nextID "${_nextID} + 1")
    globalObjSet ("${hndl}_NEXT_ROWID" "${_nextID}")
endfunction()
...
function(_sql_peel_fields in_handle field_list where_list in_keywords out_list)
    ...
    globalObjGet("${in_handle}_COLUMNS" _allCols)
...
macro(_hs_sql_resolve_handle varName outHndl)
    if (NOT "${varName}" STREQUAL "")
        ...
        globalObjIsSet("${_input}_TYPE" _exists)
        if (_exists)
            set(${outHndl} "${_input}")
        else ()
            # 2. Check if _input is a label
            globalObjGet("HS_LABEL_TO_HNDL_${_input}" _hndl)
            if (NOT _hndl)
                # Try root namespace for labels
                get_property(_current_ns GLOBAL PROPERTY HS_OBJ_CURRENT_NAMESPACE)
                if (_current_ns)
                    set_property(GLOBAL PROPERTY HS_OBJ_CURRENT_NAMESPACE "")
                    globalObjGet("HS_LABEL_TO_HNDL_${_input}" _hndl)
                    set_property(GLOBAL PROPERTY HS_OBJ_CURRENT_NAMESPACE "${_current_ns}")
                endif ()
            endif ()
...
function(CREATE)
    ...
    if (CREATE_TABLE OR CREATE_SHEET)
        globalObjSet ("${_resolvedHndl}_TYPE" "TABLE")
        _hs_sql_fields_to_storage(_label _encLabel)
        globalObjSet ("${_resolvedHndl}_LABEL" "${_encLabel}")

        _list_from_bracketed_list("${CREATE_COLUMNS}" _1 OFF)

        globalObjSet ("${_resolvedHndl}_COLUMNS" "${_1}")
        globalObjSet ("${_resolvedHndl}_ROW_COUNT" 0)
        globalObjSet ("${_resolvedHndl}_NEXT_ROWID" 1)
        globalObjSet ("${_resolvedHndl}_ROWIDS" "")
        globalObjSet ("${_resolvedHndl}_ROW_NAMES" "")

        if (CREATE_ROWS)
            foreach (_rname IN LISTS CREATE_ROWS)
                globalObjGet("${_resolvedHndl}_NEXT_ROWID" _nextID)
                ...
                # Map name to ID
                globalObjSet ("${_resolvedHndl}_ROWNAME_TO_ID_${_rname}" "${_nextID}")
                globalObjSet ("${_resolvedHndl}_ROWID_TO_NAME_${_nextID}" "${_rname}")
                globalObjAppendUnique("${_rname}" "${_resolvedHndl}_ROW_NAMES")
            endforeach ()
        endif ()

    elseif (CREATE_VIEW)
        globalObjSet ("${_resolvedHndl}_TYPE" "VIEW")
        _hs_sql_field_to_storage(_label _encLabel)
        globalObjSet ("${_resolvedHndl}_LABEL" "${_encLabel}")
        _hs_sql_list_to_record(_members _encMems)
        globalObjSet ("${_resolvedHndl}_MEMBERS" "${_encMems}")

    elseif (CREATE_MATERIALIZED_VIEW)
        globalObjSet ("${_resolvedHndl}_TYPE" "MATERIALIZED_VIEW")
        _hs_sql_field_to_storage(_label _encLabel)
        globalObjSet ("${_resolvedHndl}_LABEL" "${_encLabel}")
        ...
        foreach (_m IN LISTS _members)
            globalObjGet("${_m}_ROWIDS" _mEncIDs)
            _hs_sql_record_to_list(_mEncIDs _mIDs)
            globalObjGet("${_m}_COLUMNS" _mEncCols)
            ...
            foreach (_rid IN LISTS _mIDs)
                ...
                foreach (_c IN LISTS _allCols)
                    if (_c IN_LIST _mCols)
                        globalObjGet("${_m}_R${_rid}_${_c}" _vEnc)
    ...
    elseif (CREATE_MAP)
        globalObjSet ("${_resolvedHndl}_TYPE" "MAP")
        _hs_sql_field_to_storage(_label _encLabel)
        globalObjSet ("${_resolvedHndl}_LABEL" "${_encLabel}")
        globalObjSet ("${_resolvedHndl}_KEYS" "")
    else ()
...
function(UPDATE)
    ...
    if (_upd_args MATCHES "^([^ ]+) LABEL (.+)$")
        ...
        # Get old label
        globalObjGet("${_h}_LABEL" _oldEncLabel)
        _hs_sql_field_to_user(_oldEncLabel _oldLabel)

        # Remove old mapping if it points to this handle
        globalObjGet("HS_LABEL_TO_HNDL_${_oldLabel}" _oldHndl)
        if ("${_oldHndl}" STREQUAL "${_h}")
            globalObjSet ("HS_LABEL_TO_HNDL_${_oldLabel}" "")
        endif ()

        # Add new mapping
        _hs_sql_field_to_storage(_new_label _newEncLabel)
        globalObjSet ("${_h}_LABEL" "${_newEncLabel}")

        globalObjGet("HS_LABEL_TO_HNDL_${_new_label}" _prev)
        ...
        globalObjSet ("HS_LABEL_TO_HNDL_${_new_label}" "${_h}")
        return()
    endif ()
    ...
    if (_upd_args MATCHES "^([^ ]+) ROWNAMES (.+)$")
        ...
        globalObjGet("${_h}_ROWIDS" _rowids)
        set(_idx 0)
        ...
        foreach (_rname IN LISTS _names)
            if (_idx LESS _len)
                ...
                globalObjSet ("${_h}_ROWNAME_TO_ID_${_rname}" "${_rid}")
                globalObjSet ("${_h}_ROWID_TO_NAME_${_rid}" "${_rname}")
                globalObjAppendUnique("${_rname}" "${_h}_ROW_NAMES")
            endif ()
    ...
    if (_type STREQUAL "MAP")
        ...
        foreach (_tok IN LISTS _set_tokens)
            ...
            elseif (_tok STREQUAL "=")
                ...
                globalObjSet ("${_h}_K_${_current_key}" "${_encVal}")
                globalObjSet ("${_h}_K_${_current_key}_ISHANDLE" FALSE)
                set(_expect_key TRUE)
            endif ()
        endforeach ()
        return()
    ...
    elseif (DEFINED _where_ROW)
        globalObjGet("${_h}_ROWNAME_TO_ID_${_where_ROW}" _rid)
        if (_rid)
            list(APPEND _target_rows "${_rid}")
        endif ()
    else ()
        # No specific row — need to filter all rows
        globalObjGet("${_h}_ROWIDS" _encIDs)
        ...
        foreach (_rid IN LISTS _all_ids)
            ...
            if (_filterCount GREATER 0)
                ...
                foreach (_fIdx RANGE ${_maxF})
                    ...
                    globalObjGet("${_h}_R${_rid}_${_fCol}" _val)
    ...
    foreach (_rid IN LISTS _target_rows)
        foreach (_idx RANGE ${_max_idx})
            ...
            globalObjSet ("${_h}_R${_rid}_${_col}" "${_enc}")
        endforeach ()
    endforeach ()
...
function(INSERT)
    ...
    if (_type STREQUAL "MAP")
        ...
        foreach (_idx RANGE ${_max_idx})
            ...
            globalObjSet ("${_h}_K_${_key}" "${_encVal}")
            globalObjSet ("${_h}_K_${_key}_ISHANDLE" FALSE)
            globalObjAppendUnique("${_key}" "${_h}_KEYS")
        endforeach ()
        return()
    endif ()
    ...
    # ── Handle TABLE type ────────────────────────────────────────────────
    # Get table columns
    globalObjGet("${_h}_COLUMNS" _encCols)
    ...
    # Add row ID to list
    globalObjAppendUnique("${_h}_ROWIDS" "${_new_id}")

    if(_row_name)
        globalObjAppendUnique("${_h}_ROWNAMES" "${_row_name}")
        globalObjSet ("${_h}_ROWNAME_TO_ID_${_row_name}" ${_new_id})
    endif ()
    ...
    if (_explicit_cols)
        foreach (_col IN LISTS _table_cols)
            ...
            if (_found EQUAL -1)
                globalObjSet ("${_h}_R${_new_id}_${_col}" "${_TOK_EMPTY_FIELD}")
            endif ()
        endforeach ()
    endif ()
...
function(SELECT)
    ...
    if (_sel_args MATCHES "^(.*) FROM ([^ ]+)$")
        set(_sel_mode_cols "${CMAKE_MATCH_1}")
    ...
    if (_type STREQUAL "MAP")
        ...
        globalObjGet("${_h}_K_${_targetKey}" _raw)
    ...
    if (_type STREQUAL "VIEW")
        globalObjGet("${_h}_MEMBERS" _encMems)
    ...
    globalObjGet("${_h}_ROWNAMES" _encNames)
    ...
    if (SELECT_WHERE_ROW)
        globalObjGet("${_h}_ROWNAME_TO_ID_${SELECT_WHERE_ROW}" _rid)
        if (_rid)
            set(_tRow "${_rid}")
        else ()
            set(SELECT_ERROR "ROWNAME \"${SELECT_WHERE_ROW}\" not found" PARENT_SCOPE)
            if(SELECT_VALUE)
                _parent_scope("${SELECT_VALUES}" "")
            else ()
                _parent_scope("" "NOTFOUND")
            endif ()
            return()
        endif ()
    endif ()
    ...
    if (SELECT_STAR AND _tRow)
        ...
        foreach (_c IN LISTS _allCols)
            globalObjGet("${_h}_R${_tRow}_${_c}" _raw)
            _hs_sql_field_to_user(_raw _val)
            # Use MAP-specific insert logic
            _hs_sql_field_to_storage(_val _encVal)
            globalObjSet ("${_hMapRes}_K_${_c}" "${_encVal}")
            globalObjSet ("${_hMapRes}_K_${_c}_ISHANDLE" FALSE)
            globalObjAppendUnique("${_c}" "${_hMapRes}_KEYS")
        endforeach ()
    ...
    if ((SELECT_VALUE OR SELECT_HANDLE OR SELECT_ROW) AND _tRow)
        ...
        elseif (_tCol)
            globalObjGet("${_h}_R${_tRow}_${_tCol}" _raw)
            _hs_sql_field_to_user(_raw _final)
    ...
    if (SELECT_VALUE)
        ...
        foreach (_tCol IN LISTS SELECT_VALUES)
            ...
            foreach (_rid IN LISTS _matches)
                globalObjGet("${_h}_R${_rid}_${_tCol}" _val)
    ...
    if (SELECT_ROW)
        ...
        foreach (_col IN LISTS _allCols)
            globalObjGet("${_h}_R${_targetRow}_${_col}" _v)
    ...
    if(_matches)
        ...
        foreach (_mID IN LISTS _matches)
            set(_rowValues "")
            foreach (_col IN LISTS _allCols)
                globalObjGet("${_h}_R${_mID}_${_col}" _val)
...
function(DELETE)
    ...
    if (_rid)
        ...
        globalObjGet("${_h}_ROWIDS" _ids)
        list(REMOVE_ITEM _ids "${_rid}")
        globalObjSet ("${_h}_ROWIDS" "${_ids}")

        # Update Count
        list(LENGTH _ids _newCount)
        globalObjSet ("${_h}_ROW_COUNT" "${_newCount}")
...
function(DROP)
    ...
    globalObjGet("${_h}_TYPE" _actualType)
    if (_type_constraint)
            ...
            if (NOT "${_otherH}" STREQUAL "${_h}")
                ...
                if (_oType STREQUAL "MAP")
                    ...
                    foreach (_k IN LISTS _keysToRemoval)
                        globalObjSet ("${_otherH}_K_${_k}" "")
                        globalObjSet ("${_otherH}_K_${_k}_ISHANDLE" "")
                        list(REMOVE_ITEM _keys "${_k}")
                    endforeach ()
                    globalObjSet ("${_otherH}_KEYS" "${_keys}")
                endif ()
                if (_oType STREQUAL "VIEW")
                    globalObjGet("${_otherH}_MEMBERS" _encMems)
                    _hs_sql_record_to_list(_encMems _mems)
                    if ("${_h}" IN_LIST _mems)
                        list(REMOVE_ITEM _mems "${_h}")
                        _hs_sql_list_to_record(_mems _encNewMems)
                        globalObjSet ("${_otherH}_MEMBERS" "${_encNewMems}")
                    endif ()
                endif ()
            endif ()
        endforeach ()
    ...
    if (_type MATCHES "TABLE|MATERIALIZED_VIEW")
        ...
        foreach (_rid IN LISTS _ids)
            foreach (_c IN LISTS _cols)
                globalObjSet ("${_h}_R${_rid}_${_c}" "")
            endforeach ()
        endforeach ()
        globalObjSet ("${_h}_ROWIDS" "")
        globalObjSet ("${_h}_COLUMNS" "")
        globalObjSet ("${_h}_ROW_COUNT" "")
        globalObjSet ("${_h}_NEXT_ROWID" "")

        # Clean row name mappings if any
        globalObjGet("${_h}_ROW_NAMES" _rnames)
        foreach (_rn IN LISTS _rnames)
            globalObjSet ("${_h}_ROWNAME_TO_ID_${_rn}" "")
            globalObjSet ("${_h}_ROWID_TO_NAME_${_rn}" "")
        endforeach ()
        globalObjSet ("${_h}_ROW_NAMES" "")

    elseif (_type STREQUAL "MAP")
        globalObjGet("${_h}_KEYS" _keys)
        foreach (_k IN LISTS _keys)
            globalObjSet ("${_h}_K_${_k}" "")
            globalObjSet ("${_h}_K_${_k}_ISHANDLE" "")
        endforeach ()
        globalObjSet ("${_h}_KEYS" "")
    elseif (_type STREQUAL "VIEW")
        globalObjSet ("${_h}_MEMBERS" "")
    endif ()

    globalObjSet ("${_h}_TYPE" "")
    globalObjSet ("${_h}_LABEL" "")

    # Remove Label Mapping
    if (_label)
        globalObjGet("HS_LABEL_TO_HNDL_${_label}" _registeredHndl)
        if ("${_registeredHndl}" STREQUAL "${_h}")
            globalObjSet ("HS_LABEL_TO_HNDL_${_label}" "")
        endif ()
    endif ()
...
function(DESCRIBE)
    ...
    globalObjGet("${_h}_TYPE" _type)
    globalObjGet("${_h}_LABEL" _rawLabel)
    ...
    if (_type STREQUAL "TABLE" OR _type STREQUAL "MATERIALIZED_VIEW")
        globalObjGet("${_h}_COLUMNS" _rawCols)
        _hs_sql_record_to_list(_rawCols _cols)
        globalObjGet("${_h}_ROW_COUNT" _count)
        msg("Rows:   ${_count}")
        msg("Cols:   ${_cols}")
    elseif (_type STREQUAL "VIEW")
        globalObjGet("${_h}_MEMBERS" _rawMems)
...
function(ASSERT)
    ...
    if (NOT "${_expectedCount}" STREQUAL "")
        globalObjGet("${_h}_ROW_COUNT" _actualCount)
...
function(DUMP)
    ...
    if (_type STREQUAL "VIEW")
        globalObjGet("${_h}_MEMBERS" _encMembers)
    ...
    elseif (_type STREQUAL "MAP")
        ...
        foreach (_pass RANGE 1 2)
            foreach (_k IN LISTS _keys)
                globalObjGet("${_h}_K_${_k}" _raw)
                _hs_sql_field_to_user(_raw _v)
                globalObjGet("${_h}_K_${_k}_ISHANDLE" _isH)
                if (_pass EQUAL 1)
                    if (_isH)
    ...
    elseif (_type MATCHES "TABLE|MATERIALIZED_VIEW")
        ...
        if (NOT _verbose)
            ...
            set(_tcol "${_cols}")
            list(PREPEND _tcol Row "RowName")

            foreach (_c IN LISTS _tcol)
            ...
            foreach(_rname IN LISTS _names)
                ...
                if (_rnLen GREATER _w_RowName)
                    set(_w_RowName ${_rnLen})
                endif ()
            endforeach ()

            foreach (_rid IN LISTS _ids)
                foreach (_c IN LISTS _cols)
                    globalObjGet("${_h}_R${_rid}_${_c}" _encVal)
                    _hs_sql_field_to_user(_encVal _val)
                    _hs_sql_record_to_list(_val _valList)

                    # Apply GitHub domain omission logic for measurement
                    if (_valList MATCHES "^https://github.com/(.*)")
                        set(_valList "${CMAKE_MATCH_1}")
                    endif ()
                    ...
                    list(LENGTH _lineList _elements)
                    if(_elements GREATER_EQUAL 2 AND NOT _w_${_c}_reset)
                        # Reset the column width to just the header column width now we know it'll be much narrower
                        set(_column_name "_hw_${_c}")
                        set(_header_column_width "${${_column_name}}")
                        set(_w_${_c} ${_header_column_width})
                        set(_w_${_c}_reset ON)
                    endif ()
            ...
            if(_names)
                _hs_pad_string("Row Name" ${_w_RowName} _padded)
                string(APPEND _header " ${_padded} |")
            endif ()
            ...
            foreach (_rid IN LISTS _ids)
                ...
                foreach (_c IN LISTS _cols)
                    ...
                    _hs_sql_field_to_user(_encVal _val)
                    _hs_sql_record_to_list(_val _valList)
                    string(REPLACE "${_TOK_LIST_SEP}" ";" _noAmp "${_val}")
                    string(REPLACE ";" "\n" _lines "${_noAmp}")
                    string(REPLACE "\n" ";" _lineList "${_lines}")
                ...
                foreach (_li RANGE ${_maxLineIdx})
                    ...
                    set(_ridentifier "${_rid}")
                    if (_li EQUAL 0)
                        _hs_pad_string("${_ridentifier}" ${_w_Row} _ridentifier)
                    else ()
                        _hs_pad_string("" ${_w_Row} _ridentifier)
                    endif ()
                    string(APPEND _lineStr " ${_ridentifier} |")

                    if(_names)
                        list(GET _names ${_ridz} _ridentifier)
                        if (_li EQUAL 0)
                            _hs_pad_string("${_ridentifier}" ${_w_RowName} _ridentifier)
                        else ()
                            _hs_pad_string("" ${_w_RowName} _ridentifier)
                        endif ()
                        string(APPEND _lineStr " ${_ridentifier} |")
                    endif ()
                    ...
                    foreach (_c IN LISTS _cols)
                        ...
                        string(REPLACE "&&" "${_TOK_LIST_SEP}" _single_amp "${_maybe_blank}")
                        string(REPLACE "${_TOK_LIST_SEP}" ";" _no_amp "${_single_amp}")
                        string(REPLACE ";" "\n" _lines "${_no_amp}")
                        string(REPLACE "\n" ";" _lineList "${_lines}")
...
function(_hs_sql__iter_rows hndl outVar)
    globalObjGet("${hndl}_ROWIDS" _ids)
    set(${outVar} "${_ids}" PARENT_SCOPE)
endfunction()
...
function(_hs_sql__foreach iterType hndlValue functionName)
    ...
    elseif (iterType STREQUAL "HANDLE")
        globalObjGet("${hndlValue}_TYPE" _type)
        if (_type STREQUAL "MAP")
            globalObjGet("${hndlValue}_KEYS" _keys)
            foreach (_k IN LISTS _keys)
                globalObjGet("${hndlValue}_K_${_k}_ISHANDLE" _isH)
                if (_isH)
                    globalObjGet("${hndlValue}_K_${_k}" _hv)
    ...
    elseif (iterType STREQUAL "KEY")
        ...
        if (_type STREQUAL "MAP")
            globalObjGet("${hndlValue}_KEYS" _keys)
            foreach (_k IN LISTS _keys)
                globalObjGet("${hndlValue}_K_${_k}" _raw)
