...
function(scanLibraryTargets libName packageData)
    ...
    if (libs)
        ...
        foreach(lib IN LISTS libs)
            ...
            if ("${clean_lib}" STREQUAL "" OR "${clean_lib}" IN_LIST alreadyLocated)
                continue()
            endif()
            ...
            while(s_at LESS s_max)
                ...
                if ("${raw_import_name}" STREQUAL "${pkg_name}" OR "${raw_import_name}" STREQUAL "${ns}")
                    set(smoochie "${smoochie} ${BOLD}${GREEN}YES!${NC}")
                    set(MATCHED ON)
                elseif(components)
                ...
                if (MATCHED)
                    if(ns STREQUAL "")
                        if(matching_component)
                            message(STATUS "    -> Feature '${feat_name}' (${pkg_name}::${matching_component}) is already provided by ${targetName} as ${clean_lib}")
                        else ()
                            message(STATUS "    -> Feature '${feat_name}' (${pkg_name}) is already provided by ${targetName} as ${clean_lib}")
                        endif ()
                    else ()
                        if(matching_component)
                            message(STATUS "    -> Feature '${feat_name}' (${ns}::${matching_component}) is already provided by ${targetName} as ${clean_lib}")
                        else ()
                            message(STATUS "    -> Feature '${feat_name}' (${pkg_name}) is already provided by ${targetName} as ${clean_lib}")
                        endif ()
                    endif ()
                    ...
                    list(APPEND alreadyLocated "${clean_lib}")
                    set(alreadyLocated "${alreadyLocated}" CACHE INTERNAL "")
                    break()
                endif()
            endwhile ()
            msg()
