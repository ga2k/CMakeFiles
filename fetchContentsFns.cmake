...
# @formatter:off
set(PkgColNames FeatureName PackageName Namespace Kind Method Url GitRepository GitTag SrcDir BuildDir IncDir Components Args Prereq)
set(FIXName          0)
set(FIXPkgName       1)
set(FIXNamespace     2)
set(FIXKind          3)
...
#CREATE(TABLE tbl_ColumnNames)
#INSERT(INTO  tbl_ColumnNames AS FieldWidths VALUES VERB OBJECT SUBJECT_PREP SUBJECT ITEM_PREP ITEM HANDLER)
#INSERT(INTO  tbl_ColumnNames AS PkgColNames FeatureName PackageName Namespace Kind Method Url GitRepository GitTag SrcDir BuildDir IncDir Components Args Prereq)
#
#CREATE( TABLE tbl_LongestStrings
#        COLUMNS ("SELECT ROW FROM hColumnNames WHERE ROW = FieldWidths")
#        INTO hLongest
#)
CREATE( TABLE tbl_LongestStrings
...
function(initialiseFeatureHandlers DRY_RUN)
    ...
    foreach (handler IN LISTS handlers)
        ...
        SELECT(VALUE FROM tbl_LongestStrings WHERE ROWID = 1 AND COLUMN = HANDLER INTO _longest)
        longest(RIGHT GAP
                CURRENT ${_longest}
                TEXT "${handlerName}"
                LONGEST _longest
...
function(addPackageData)
    ...
    if (NOT APD_DRY_RUN)
        CREATE(TABLE ${APD_PKGNAME} COLUMNS "${PkgColNames}" INTO hOutput)
    ...
    function(createOrAppendTo)
        ...
        function(_ VERB OBJECT SUBJECT_PREP SUBJECT ITEM_PREP ITEM TEMPLATE)
            macro(_doLine _var_)
                ...
                SELECT(VALUE FROM tbl_LongestStrings WHERE ROWID = 1 AND COLUMN ${current_tag} INTO _longest)
                ...
                UPDATE(tbl_LongestStrings COLUMN ${current_tag} SET "${_longest}" WHERE ROWID = 1)
            endmacro()
        ...
        set(args TARGET SUBJECT FEATURE RECORD TEMPLATE)
        set(lists "FIELDS")
        ...
        if (CAT_FIELDS AND NOT "${CAT_FIELDS}" STREQUAL "")
            set(cat_data "${CAT_FIELDS}")
            set(cat_record OFF)
            set(cat_fields ON)
        endif ()
        ...
        if (cat_fields)
            ...
            list(LENGTH cat_data n)
            if (n GREATER 1)
                list(GET cat_data 0 cat_feature_name)
                list(GET cat_data 1 cat_package_name)
            else ()
                set(cat_feature_name "${cat_data}")
            endif ()
            set(package_field)

            SELECT(VALUE FROM ${cat_subject_handle} WHERE COLUMN "Name" = "${cat_feature_name}" INTO feature_field)
            #            if(cat_package_name)
            #                SELECT(VALUE FROM ${cat_subject_handle} WHERE COLUMN "DfltPkg" = "${cat_package_name}" INTO package_field)
            #            endif ()

            set(fully OFF)
            #            if(cat_feature_name AND feature_field AND (NOT cat_package_name OR (cat_package_name AND package_field)))
            if (cat_feature_name AND feature_field)
                set(fully ON)
            endif ()

            if ((fully AND cat_unique) OR (fully AND NOT cat_unique AND NOT cat_extend))
                set(out_verb "skipped")
                set(out_subject_prep "for")
                if (APD_DRY_RUN)
                    list(APPEND out_verb skipped added extended created)
                    list(APPEND out_subject_prep for with to)
                endif ()
                set(out_object "duplicate")
                set(out_subject "${cat_subject}")
                set(out_item_prep "in")
                set(out_item ${cat_target})
            elseif ((fully AND NOT cat_unique AND cat_extend) OR NOT fully)
                if (NOT APD_DRY_RUN)
                ...
                if (APD_DRY_RUN)
                    list(APPEND out_verb added skipped extended created)
                    list(APPEND out_subject_prep to for with)
                endif ()
                ...
                set(out_subject_prep "for")
                if (APD_DRY_RUN)
                    list(APPEND out_verb added extended created)
                    list(APPEND out_subject_prep for with to)
                endif ()
                set(out_object "duplicate")
                set(out_subject "${cat_subject}")
            ...
            if (NOT h)
                ...
                if (NOT APD_DRY_RUN)
                    CREATE(TABLE ${cat_feature} COLUMNS "${PkgColNames}" INTO h)
                    SELECT(ROW FROM ${cat_data} WHERE ROWID 1 INTO newRow)
                    INSERT(INTO h VALUES ${newRow})
                    INSERT(INTO ${cat_target_handle} KEY ${cat_feature} HANDLE h)
                endif ()
                set(out_verb "created")
                set(out_subject_prep "for")
                if (APD_DRY_RUN)
                    list(APPEND out_verb added extended skipped)
                    list(APPEND out_subject_prep for with to)
                endif ()
            else ()
                if (NOT APD_DRY_RUN)
                ...
                if (APD_DRY_RUN)
                    list(APPEND out_verb added created skipped)
                    list(APPEND out_subject_prep with for to)
                endif ()
            endif ()
    ...
    set(template "[VERB:R][OBJECT:L][SUBJECT_PREP:R][SUBJECT:L][ITEM_PREP:R][ITEM:L]")
    ...
    # @formatter:off
    createOrAppendTo(                 TARGET h${root} FEATURE "${APD_FEATURE}" RECORD hOutput                                                TEMPLATE "${template}")
    createOrAppendTo(h${root}Packages TARGET h${root} FEATURE "${APD_FEATURE}" FIELDS "${APD_FEATPKG}"                UNIQUE  TEMPLATE "${template}")
    createOrAppendTo(h${root}Features TARGET h${root} FEATURE "${APD_FEATURE}" FIELDS "${APD_FEATURE};${APD_PKGNAME}" UNIQUE  TEMPLATE "${template}")
    # @formatter:on
...
function(getFeaturePackage hSource feature index receivingVarName)
    ...
    if (type STREQUAL "MAP" OR type STREQUAL "CATALOG")
...
function(getFeaturePackageByName hSource feature name receivingVarName)
    ...
    getObjectType(${hSource} type)
    # Returns: RECORD | ARRAY_RECORDS | ARRAY_ARRAYS | MAP | UNSET | UNKNOWN
...
function(parsePackage)
    ...
    macro(inputTypeToObjectType)
        if (A_PP_INPUT_TYPE STREQUAL "SET" OR A_PP_INPUT_TYPE STREQUAL "MAP")
            set(inputTypeToType "MAP")
            set(A_SET ON)
    ...
    if (deducedType STREQUAL "MAP" OR
            deducedType STREQUAL "ARRAY_RECORDS" OR
            deducedType STREQUAL "RECORD")
        set(A_PP_INPUT_TYPE ${deducedType})
        inputTypeToObjectType()
    else ()
        set(inputListVerifyFailed "Type of object stored in ${BOLD}${inputListName}${NC} cannot be used. Try a MAP, ARRAY_RECORDS, or RECORD")
    endif ()
...
function(preProcessFeatures featureList hDataSource outVar)
    ...
    foreach (feature IN LISTS featureList)
        ...
        set(hPackage)
        SELECT(* FROM ${hDataSource} WHERE KEY = ${_feature} INTO hCompleteFeature)
        if (hCompleteFeature)
            if (AA_PACKAGE)
                SELECT(* FROM hCompleteFeature WHERE NAME = ${_pkg} INTO hPackage)
                SELECT(VALUE FROM hPackage WHERE COLUMN = PackageName INTO reqdPkgName)
            else ()
                SELECT(* FROM hCompleteFeature WHERE ROWID = 1 INTO hPackage)
                SELECT(VALUE FROM hPackage WHERE COLUMN = PackageName INTO defPkgName)
                set(_pkg "${defPkgName}")
            endif ()
            if (NOT defPkgName)
                SELECT(* FROM hCompleteFeature WHERE ROWID = 1 INTO tPkg)
                SELECT(VALUE FROM hPackage WHERE COLUMN = PackageName INTO defPkgName)
            endif ()
        endif ()
        ...
        if (NOT hPackage)
            if (AA_NAME)
                msg(ALWAYS FATAL_ERROR "preProcessFeatures: Feature/Package \"${feature}/${_pkg}\" does not exist")
            else ()
        ...
        #            set(USING_${__temp_pkg} ON)
        #            continue()
        #        endif ()
        #
        #        SELECT(VALUE FROM hPackage WHERE COLUMN = Kind INTO _kind)
        #        if (_kind STREQUAL "SYSTEM" AND _pkg STREQUAL "${defPkgName}" AND AA_PACKAGE)
        #            # Don'd add this
        #            continue()
        #        endif ()
