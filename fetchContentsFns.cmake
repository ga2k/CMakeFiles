...
#    endif ()
    ...
    getFeatureIndex(FEATURES ${APD_FEATURE} pkgIndex)
    ...
    if (NOT DEFINED MAX_FEATURE_LENGTH)
        ...
        set (MAX_FEATURE_LENGTH "${MAX_FEATURE_LENGTH}" PARENT_SCOPE)
    endif ()
    if(${this_feature_length} GREATER ${MAX_FEATURE_LENGTH})
        set (MAX_FEATURE_LENGTH "${this_feature_length}")
        set (MAX_FEATURE_LENGTH "${MAX_FEATURE_LENGTH}" PARENT_SCOPE)
    endif ()
    ...
    if (${pkgIndex} EQUAL -1)
        ...
        record(DUMP output)
        array(APPEND newFeature RECORD "${output}")
        array(APPEND FEATURES ARRAY "${newFeature}")
        set(${FEATURES} "${FEATURES}" PARENT_SCOPE)
        msg("Adding feature${padding}${BOLD}${APD_FEATURE}${NC} for package ${BOLD}${APD_PKGNAME}${NC}")
    else ()
        array(GET FEATURES ${pkgIndex} existing_feature)
        array(APPEND existing_feature RECORD ${output})
        array(SET FEATURES ${pkgIndex} ARRAY "${existing_feature}")
        set(FEATURES "${FEATURES}" PARENT_SCOPE)
...
function(getFeatureIndex arrayName feature receivingVarName)
    # iterate over array to find line with feature
#    foreach(arr IN LISTS arrayName)
#        array(FIND ${arrayName} ${FeatureIX} MATCHING "${feature}" ix )
#        if (ix >= 0)
...
#    endforeach ()
    ...
    set(ix 0)
    array(LENGTH FEATURES numFeatures)
    if(numFeatures GREATER 0)
        foreach (index RANGE ${numFeatures})
            array(GET FEATURES ${index} thisFeature)
            array(FIND thisFeature ${FeatureIX} MATCHING "${feature}" tempe)
            if (tempe GREATER_EQUAL 0)
                set(${receivingVarName} ${ix} PARENT_SCOPE)
                return()
            endif ()
            math(EXPR ix "${ix} + 1")
...
function(processFeatures featureList returnVarName)
    ...
    foreach(feature IN LISTS featureList)
        ...
        list(APPEND revised_features "${output}")
