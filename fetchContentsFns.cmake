...
#           Keyword         Type        Direction   Description
            ...
            BIN_DIR     #   VARNAME     OUT         Receive a copy of the BINDIR attribute (if applicable)
    ...
    list(GET local ${FIXBinDir}         localBinDir)
    ...
    if (DEFINED A_PP_SRC_DIR)
        set(${A_PP_SRC_DIR}     "${localSrcDir}"    PARENT_SCOPE)
    endif ()
    if (DEFINED A_PP_BIN_DIR)
        set(${A_PP_BIN_DIR}     "${localBinDir}"    PARENT_SCOPE)
    endif ()
    if (DEFINED A_PP_INC_DIR)
        set(${A_PP_INC_DIR}     "${localIncDir}"    PARENT_SCOPE)
    endif ()
    ...
    if (A_PP_BIN_DIR AND localBinDir)
        string(FIND "${localBinDir}" "[" open_bracket)
        string(FIND "${localBinDir}" "]" close_bracket)
        if (open_bracket GREATER_EQUAL 0 AND close_bracket GREATER ${open_bracket})
            math(EXPR one_past_open_bracket "${open_bracket} + 1")
            math(EXPR one_before_close_bracket "${close_bracket} - 1")
            math(EXPR one_past_close_bracket "${close_bracket} + 1")
            math(EXPR dirroot_length "${one_before_close_bracket} - ${one_past_open_bracket} + 1")
            string(SUBSTRING "${localBinDir}" ${one_past_open_bracket} ${dirroot_length} dirroot)
            string(SUBSTRING "${localBinDir}" ${one_past_close_bracket} -1 src_folder)

            if (${dirroot} STREQUAL "SRC")
                set(${A_PP_BIN_DIR} ${EXTERNALS_DIR}/${src_folder} PARENT_SCOPE)
                set(build_ok ON)
            elseif (${dirroot} STREQUAL "BUILD")
                set(${A_PP_BIN_DIR} ${BUILD_DIR}/${src_folder} PARENT_SCOPE)
                set(build_ok ON)
            else ()
                msg(ALWAYS FATAL_ERROR "Bad BINDIR for ${this_pkgname} (${localBinDir}): Must start with \"[SRC]\" or \"[BUILD]\"")
            endif ()
        else ()
            set(build_ok ON)
        endif ()
...
#    endif ()
    ...
    if (A_PP_INC_DIR)
        ...
        if (open_bracket GREATER_EQUAL 0 AND close_bracket GREATER ${open_bracket})
            ...
            if (${dirroot} STREQUAL "SRC")
                set(${A_PP_INC_DIR} ${EXTERNALS_DIR}/${src_folder} PARENT_SCOPE)
                set(inc_ok ON)
            elseif (${dirroot} STREQUAL "BUILD")
                set(${A_PP_INC_DIR} ${BUILD_DIR}/${src_folder} PARENT_SCOPE)
                set(inc_ok ON)
            else ()
...
macro(handleTarget _pkgname)
    ...
    if (this_inc)
        if (EXISTS "${this_inc}")
            list(APPEND _IncludePathsList ${this_inc})
        endif ()
    else ()
    ...
    function(subProcess subArgs retVar)
        ...
        if (AA1_PATHS OR "PATHS" IN_LIST AA1_KEYWORDS_MISSING_VALUES)
            if (NOT "${AA1_PATHS}" STREQUAL "")
                replacePositionalParameters("${AA1_PATHS}" _paths0 ON)
                if (_paths0)
                    string(JOIN "${_TOK_LIST_SEP}" _paths PATHS ${_paths0})
                else ()
