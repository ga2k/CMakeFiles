...
CREATE(TABLE hLongest AS tbl_LongestStrings COLUMNS "VERB;OBJECT;SUBJECT_PREP;SUBJECT;ITEM_PREP;ITEM;HANDLER")
...
function(addPackageData)
    ...
    if (NOT APD_DRY_RUN)
        CREATE(TABLE hOutput AS ${APD_PKGNAME} COLUMNS "${PkgColNames}")
...
#            if(cat_feature_name AND feature_field AND (NOT cat_package_name OR (cat_package_name AND package_field)))
            ...
            if (NOT h)
                ...
                if (NOT APD_DRY_RUN)
                    CREATE(TABLE h AS ${cat_feature} COLUMNS "${PkgColNames}")
...
function(getFeaturePackage hSource feature index receivingVarName)
    ...
    if (type STREQUAL "DICTIONARY" OR type STREQUAL "CATALOG")
...
function(getFeaturePackageByName hSource feature name receivingVarName)
    ...
    getObjectType(${hSource} type)
    # Returns: RECORD | ARRAY_RECORDS | ARRAY_ARRAYS | DICTIONARY | UNSET | UNKNOWN
...
function(parsePackage)
    ...
    macro(inputTypeToObjectType)
        if (A_PP_INPUT_TYPE STREQUAL "SET" OR A_PP_INPUT_TYPE STREQUAL "DICTIONARY")
            set(inputTypeToType "DICTIONARY")
            set(A_SET ON)
    ...
    if (deducedType STREQUAL "DICTIONARY" OR
            deducedType STREQUAL "ARRAY_RECORDS" OR
            deducedType STREQUAL "RECORD")
        set(A_PP_INPUT_TYPE ${deducedType})
        inputTypeToObjectType()
    else ()
        set(inputListVerifyFailed "Type of object stored in ${BOLD}${inputListName}${NC} cannot be used. Try a DICTIONARY, ARRAY_RECORDS, or RECORD")
    endif ()
...
function(preProcessFeatures featureList hDataSource outVar)
    ...
    CREATE(TABLE hRevisedFeatures AS tblRevisedFeatures COLUMNS "${PkgColNames}")
