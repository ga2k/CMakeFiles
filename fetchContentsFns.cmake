...
function(addPackageData)
    set(switches SYSTEM LIBRARY OPTION)
    ...
    if (NOT APD_SYSTEM AND NOT APD_OPTION AND NOT APD_LIBRARY)
        set(APD_OPTION ON)
    endif ()

    if ((APD_SYSTEM AND APD_OPTION) OR (APD_SYSTEM AND APD_LIBRARY) OR (APD_OPTION AND APD_LIBRARY))
        msg(ALWAYS FATAL_ERROR "addPackageData: Zero or one of SYSTEM/OPTION/LIBRARY allowed")
    else ()
        if(APD_SYSTEM)
            set(APD_KIND "SYSTEM")
        elseif(APD_LIBRARY)
                ...
                foreach(item ${placeholders})
                    if(item MATCHES "\\[([A-Z]+):([RL]),([0-9]+)\\]")
                        ...
                        if(current_tag STREQUAL "VERB")
                            if(NOT DEFINED verb_min)
                                set(verb_min ${current_width})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${verb_min} PAD_CHAR " " TEXT "${cat_verb}" PADDED VERB LONGEST verb_min)
                            if(VERB MATCHES "created")
                                set(VERB "${BOLD}${GREEN}${VERB}${NC}")
                            elseif(cat_verb STREQUAL "added")
                                set(VERB "${BOLD}${VERB}${NC}")
                            elseif(cat_verb STREQUAL "replaced")
                                set(VERB "${BOLD}${RED}${VERB}${NC}")
                            elseif(cat_verb STREQUAL "extended")
                                set(VERB "${BOLD}${YELLOW}${VERB}${NC}")
                            elseif(cat_verb STREQUAL "skipped")
                                set(VERB "${BOLD}${BLUE}${VERB}${NC}")
                            endif ()
                        elseif(current_tag STREQUAL "OBJECT")
                            if(NOT DEFINED objt_min)
                                set(objt_min ${current_width})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${objt_min} PAD_CHAR " " TEXT "${cat_object}" PADDED OBJECT LONGEST objt_min)
                        ...
                        elseif(current_tag STREQUAL "PREPOSITION")
                            ...
                            if(NOT DEFINED prep_min)
                                set(prep_min ${current_width})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${prep_min} PAD_CHAR " " TEXT "${arg}" PADDED PREPOSITION LONGEST prep_min)
                            set(PREPOSITION "${BOLD}${PREPOSITION}${NC}")
                        elseif(current_tag STREQUAL "ITEM")
                            ...
                            if(NOT DEFINED item_min)
                                set(item_min ${current_width})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${item_min} PAD_CHAR " " TEXT "${arg}" PADDED ITEM LONGEST item_min)
                            set(ITEM "${BOLD}${YELLOW}${ITEM}${NC}")
                        elseif(current_tag STREQUAL "COLLECTION")
                            if(NOT DEFINED coll_min)
                                set(coll_min ${current_width})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${coll_min} PAD_CHAR " " TEXT "${cat_target}" PADDED COLLECTION LONGEST coll_min)
...
#        string(LENGTH "${plain_text}" visual_len)
    ...
    set(template "[VERB:R,8] [OBJECT:L,10] [PREPOSITION:R,4] [ITEM:L,10] in the [COLLECTION:R,17] collection")
    set(APD_FEATPKG "${APD_FEATURE}/${APD_PKGNAME}")

    # Add new feature/pkg to the global FEATURES collection
    createOrAppendTo (    "FEATURES"          FEATURE "${APD_FEATURE}" RECORD "${output}"          EXTEND        TEMPLATE "${template}")
    createOrAppendTO (    "FEATURES"          FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG}       EXTEND UNIQUE TEMPLATE "${template}")
    createOrAppendTO (    "FEATURES"          FEATURE "NAMES"          FIELD  ${APD_FEATURE}       EXTEND UNIQUE TEMPLATE "${template}")
    if(APD_KIND MATCHES "SYSTEM")
        createOrAppendTo ("SYSTEM_FEATURES"   FEATURE "${APD_FEATURE}" RECORD "${output}"          EXTEND        TEMPLATE "${template}")
        createOrAppendTO ("SYSTEM_FEATURES"   FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG}       EXTEND UNIQUE TEMPLATE "${template}")
        createOrAppendTO ("SYSTEM_FEATURES"   FEATURE "NAMES"          FIELD  ${APD_FEATURE}       EXTEND UNIQUE TEMPLATE "${template}")
    elseif(APD_KIND MATCHES "LIBRARY")
        createOrAppendTo ("LIBRARY_FEATURES"  FEATURE "${APD_FEATURE}" RECORD "${output}"          EXTEND        TEMPLATE "${template}")
        createOrAppendTO ("LIBRARY_FEATURES"  FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG}       EXTEND UNIQUE TEMPLATE "${template}")
        createOrAppendTO ("LIBRARY_FEATURES"  FEATURE "NAMES"          FIELD  ${APD_FEATURE}       EXTEND UNIQUE TEMPLATE "${template}")
    elseif(APD_KIND MATCHES "OPTIONAL")
        createOrAppendTo ("OPTIONAL_FEATURES" FEATURE "${APD_FEATURE}" RECORD "${output}"          EXTEND        TEMPLATE "${template}")
        createOrAppendTO ("OPTIONAL_FEATURES" FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG}       EXTEND UNIQUE TEMPLATE "${template}")
        createOrAppendTO ("OPTIONAL_FEATURES" FEATURE "NAMES"          FIELD  ${APD_FEATURE}       EXTEND UNIQUE TEMPLATE "${template}")
    endif()
...
#           Keyword         Type        Direction   Description
            ...
            KIND        #   VARNAME     OUT         Receive a copy of the KIND attribute (SYSTEM/LIBRARY/OPTION)
