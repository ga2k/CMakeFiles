...
set(__longest_handler "0" CACHE INTERNAL "" FORCE)
set(__longest_object "0" CACHE INTERNAL "" FORCE)
set(__longest_verb "0" CACHE INTERNAL "" FORCE)
set(__longest_preposition "0" CACHE INTERNAL "" FORCE)
set(__longest_dict "0" CACHE INTERNAL "" FORCE)
set(__longest_item "0" CACHE INTERNAL "" FORCE)
...
function(addPackageData)
    ...
    function(createOrAppendTo)
        ...
        function(_ VERB OBJECT SUBJECT_PREPOSITION SUBJECT TARGET_PREPOSITION TARGUT TEMPLATE ABBREV)
            if (NOT cat_quiet)

                if(NOT VERB OR "${VERB}" STREQUAL "")
                    set(VERB " ")
                endif ()
                if(NOT OBJECT OR "${OBJECT}" STREQUAL "")
                    set(OBJECT " ")
                endif ()
                if(NOT SUBJECT_PREPOSITION OR "${SUBJECT_PREPOSITION}" STREQUAL "")
                    set(SUBJECT_PREPOSITION " ")
                endif ()
                if(NOT SUBJECT OR "${SUBJECT}" STREQUAL "")
                    set(SUBJECT " ")
                endif ()
                if(NOT TARGET_PREPOSITION OR "${TARGET_PREPOSITION}" STREQUAL "")
                    set(TARGET_PREPOSITION " ")
                endif ()
                if(NOT TARGUT OR TARGUT STREQUAL "")
                    set(TARGUT " ")
                endif ()

                set(outstr "${TEMPLATE}")

                string(REGEX MATCHALL "\\[[A-Z]+:[^]]+\\]" placeholders "${TEMPLATE}")
                ...
                foreach (item ${placeholders})
                    if (item MATCHES "\\[([A-Z]+):([RL])\\]")
                        ...
                        # set(TEMPLATE "[VERB:R] [OBJECT:L] [PREPOSITION:R] [SUBJECT:L] [DESTINATION:R]")

                        if (current_tag STREQUAL "VERB")
                            longest(${JUSTIFY} CURRENT ${__longest_verb} PAD_CHAR " " TEXT "${VERB}" PADDED VERB LONGEST __longest_verb)
...
#                            endif ()
                        elseif (current_tag STREQUAL "OBJECT")
                            longest(${JUSTIFY} CURRENT ${__longest_object} PAD_CHAR " " TEXT "${OBJECT}" PADDED OBJECT LONGEST __longest_object)
                            set(OBJECT "${OBJECT}")
#                            set(OBJECT "${OBJECT}${NC}")
                        elseif (current_tag STREQUAL "SUBJECT_PREPOSITION")
                            if (ABBREV OR SUBJECT_PREPOSITION STREQUAL "")
                                set(arg " ")
                            else ()
                                set(arg ${SUBJECT_PREPOSITION})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${__longest_preposition} PAD_CHAR " " TEXT "${arg}" PADDED SUBJECT_PREPOSITION LONGEST __longest_preposition)
                            set(SUBJECT_PREPOSITION "${SUBJECT_PREPOSITION}")
#                            set(SUBJECT_PREPOSITION "${SUBJECT_PREPOSITION}${NC}")
                        elseif (current_tag STREQUAL "SUBJECT")
                            if (ABBREV OR SUBJECT STREQUAL "")
                                set(arg " ")
                            else ()
                                set(arg ${SUBJECT})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${__longest_item} PAD_CHAR " " TEXT "${arg}" PADDED SUBJECT LONGEST __longest_item)
                            set(SUBJECT "${SUBJECT}")
#                            set(SUBJECT "${YELLOW}${SUBJECT}${NC}")
                        elseif (current_tag STREQUAL "TARGET_PREPOSITION")
                            if (ABBREV OR TARGET_PREPOSITION STREQUAL "")
                                set(arg " ")
                            else ()
                                set(arg ${TARGET_PREPOSITION})
                            endif ()
                            longest(${JUSTIFY} CURRENT ${__longest_preposition} PAD_CHAR " " TEXT "${arg}" PADDED TARGET_PREPOSITION LONGEST __longest_preposition)
                            set(TARGET_PREPOSITION "${TARGET_PREPOSITION}${NC}")
                        elseif (current_tag STREQUAL "TARGET")
                            longest(${JUSTIFY} CURRENT ${__longest_dict} PAD_CHAR " " TEXT "${TARGUT}" PADDED TARGUT LONGEST __longest_dict)
                            set(TARGUT "${TARGUT}")
                            #                            set(TARGUT "${BLUE}${TARGUT}${NC}")
                        endif ()
                    endif ()
                endforeach ()
                string(REGEX REPLACE "\\[VERB:[^]]*\\]" "${VERB}" outstr "${outstr}")
                string(REGEX REPLACE "\\[OBJECT:[^]]*\\]" "${OBJECT}" outstr "${outstr}")
                string(REGEX REPLACE "\\[SUBJECT_PREPOSITION:[^]]*\\]" "${SUBJECT_PREPOSITION}" outstr "${outstr}")
                string(REGEX REPLACE "\\[SUBJECT:[^]]*\\]" "${SUBJECT}" outstr "${outstr}")
                string(REGEX REPLACE "\\[TARGET_PREPOSITION:[^]]*\\]" "${TARGET_PREPOSITION}" outstr "${outstr}")
                string(REGEX REPLACE "\\[TARGET:[^]]*\\]" "${TARGUT}" outstr "${outstr}")
...
#                endif ()
            ...
            set(__longest_object ${__longest_object} CACHE INTERNAL "" FORCE)
            set(__longest_verb ${__longest_verb} CACHE INTERNAL "" FORCE)
            set(__longest_preposition ${__longest_preposition} CACHE INTERNAL "" FORCE)
            set(__longest_dict ${__longest_dict} CACHE INTERNAL "" FORCE)
            set(__longest_item ${__longest_item} CACHE INTERNAL "" FORCE)
        ...
        set(cat_verb)
        set(cat_subject_preposition)
        set(cat_target_preposition)
        ...
        if (cat_field)
            ...
            if ((fieldValue AND cat_unique) OR (fieldValue AND NOT cat_unique AND NOT cat_extend))
                ...
                set(cat_subject_preposition "for")
                set(cat_target_preposition "in")
            ...
            elseif ((fieldValue AND NOT cat_unique AND cat_extend) OR NOT fieldValue)
                ...
                set(cat_verb "added")
                set(cat_object "${cat_data}")
                set(cat_subject_preposition "to")
                set(cat_target_preposition "in")
            else ()
                set(cat_verb "skipped")
                set(cat_object ${cat_data})
            ...
            _("${cat_verb}" "${cat_object}" "${cat_subject_preposition}" "${cat_subject}" "${cat_target_preposition}" "${cat_target}" "${cat_template}" OFF)
        else ()
            ...
            if (NOT h)
                ...
                if (NOT APD_DRY_RUN)
                    ...
                    set(cat_subject_preposition "feature")
                    set(cat_subject)
                    set(cat_target_preposition "in")
                    _("${cat_verb}" "${cat_object}" "${cat_subject_preposition}" "${cat_subject}" "${cat_target_preposition}" "${cat_target}" "${cat_template}" OFF)
                endif ()
            ...
            if (locHandle OR APD_DRY_RUN)
                ...
                set(cat_object ${locPkgName})
                set(cat_subject_preposition "in")
                set(cat_subject ${cat_feature})
                # Do nothing
            else ()
                if (NOT APD_DRY_RUN)
                    if (weCreatedH)
                        ...
                        object(REMOVE RECORD FROM ${cat_target} WHERE NAME EQUAL ${locPkgName} REPLACE WITH "${cat_data}" STATUS result)
                    endif ()
                endif ()
                set(cat_verb "extended")
                set(cat_object ${cat_feature})
                set(cat_subject_preposition "with")
                set(cat_subject ${locPkgName})
                if (NOT result)
                    # OOPS
                endif ()
            endif ()
        ...
        #        if(NOT cat_verb OR NOT cat_object OR NOT cat_subject_preposition OR NOT cat_subject)
        #            set(cat_verb "skipped")
        #            set(cat_object ${cat_data})
        #            set(cat_subject_preposition "in")
    ...
    set(template "[VERB:R] [OBJECT:L] [SUBJECT_PREPOSITION:R] [SUBJECT:L] [TARGET_PREPOSITION:L] [TARGET:R]")
