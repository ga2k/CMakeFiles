...
function(addPackageData)
    ...
    function(createOrAppendTo)
        ...
        function(_ VERB OBJECT SUBJECT_PREPOSITION SUBJECT TARGET_PREPOSITION TARGET TEMPLATE ABBREV)
            if (NOT cat_quiet)
                ...
                if(NOT TARGET_PREP OR "${TARGET_PREP}" STREQUAL "")
                    set(TARGET_PREP " ")
                endif ()
                ...
                foreach (item ${placeholders})
                    if (item MATCHES "\\[([A-Z]+):([RL])\\]")
                        ...
                        elseif (current_tag STREQUAL "TARGET")
                            longest(${JUSTIFY} CURRENT ${__longest_dict} PAD_CHAR " " TEXT "${SUBJECT}" PADDED DESTINATION LONGEST __longest_dict)
                            set(TARGET "${BLUE}${TARGET}${NC}")
                        endif ()
                    endif ()
                endforeach ()
                ...
                string(REGEX REPLACE "\\[TARGET:[^]]*\\]" "${TARGET}" outstr "${outstr}")
        ...
        if (cat_field)
            ...
            if (locHandle OR APD_DRY_RUN)
                ...
                if (NOT APD_DRY_RUN)
                    if (weCreatedH)
                        object(SET ${cat_target} NAME EQUAL "${cat_feature}" HANDLE "${cat_data}")
                    else ()
