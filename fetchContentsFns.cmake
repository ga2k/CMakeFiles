...
#            string(STRIP "${${current_tag}}" ${current_tag})
            ...
            if (current_tag STREQUAL "VERB")
                ...
                if(VERB MATCHES "^(.*)(created|added|replaced|extended|skipped|calling)([^ ]+)(.*)$")
                    ...
                    string(STRIP "${thing}" thing)
                else ()
#                _doLine("VERB")
                    SplitAt("${VERB}" " " action thing)
                endif ()
                if (action MATCHES "created")
                    set(VERB "${BOLD}${YELLOW}${CMAKE_MATCH_1}${action}${NC} ${thing}")
                elseif (action MATCHES "added")
                    set(VERB "${BOLD}${WHITE}${CMAKE_MATCH_1}${action}${NC} ${thing}")
                elseif (action MATCHES "replaced")
                    set(VERB "${BOLD}${RED}${CMAKE_MATCH_1}${action}${NC} ${thing}")
                elseif (action MATCHES "extended")
                    set(VERB "${BOLD}${CYAN}${CMAKE_MATCH_1}${action}${NC} ${thing}")
                elseif (action MATCHES "skipped")
                    set(VERB "${BOLD}${BLUE}${CMAKE_MATCH_1}${action}${NC} ${thing}")
                elseif (action MATCHES "calling")
                    set(VERB "${BOLD}${GREEN}${CMAKE_MATCH_1}${action}${NC} ${thing}")
                endif ()
