...
function(addPackageData)
    ...
    set(switches SYSTEM LIBRARY OPTIONAL DRY_RUN)
    ...
    if ((APD_SYSTEM AND APD_OPTION) OR (APD_SYSTEM AND APD_LIBRARY) OR (APD_OPTION AND APD_LIBRARY))
        msg(ALWAYS FATAL_ERROR "addPackageData: Zero or one of SYSTEM/OPTIONAL/LIBRARY allowed")
    else ()
        if(APD_SYSTEM)
            set(APD_KIND "SYSTEM")
        elseif(APD_LIBRARY)
...
#        string(LENGTH "${plain_text}" visual_len)
    ...
    createOrAppendTO (    "FEATURES"          FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG} QUIET EXTEND UNIQUE TEMPLATE "${template}")
    createOrAppendTO (    "FEATURES"          FEATURE "NAMES"          FIELD  ${APD_FEATURE} QUIET EXTEND UNIQUE TEMPLATE "${template}")
    if(APD_KIND MATCHES "SYSTEM")
        createOrAppendTo ("SYSTEM_FEATURES"   FEATURE "${APD_FEATURE}" RECORD "${output}"    QUIET EXTEND        TEMPLATE "${template}")
        createOrAppendTO ("SYSTEM_FEATURES"   FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG} QUIET EXTEND UNIQUE TEMPLATE "${template}")
        createOrAppendTO ("SYSTEM_FEATURES"   FEATURE "NAMES"          FIELD  ${APD_FEATURE} QUIET EXTEND UNIQUE TEMPLATE "${template}")
    elseif(APD_KIND MATCHES "LIBRARY")
        createOrAppendTo ("LIBRARY_FEATURES"  FEATURE "${APD_FEATURE}" RECORD "${output}"    QUIET EXTEND        TEMPLATE "${template}")
        createOrAppendTO ("LIBRARY_FEATURES"  FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG} QUIET EXTEND UNIQUE TEMPLATE "${template}")
        createOrAppendTO ("LIBRARY_FEATURES"  FEATURE "NAMES"          FIELD  ${APD_FEATURE} QUIET EXTEND UNIQUE TEMPLATE "${template}")
    elseif(APD_KIND MATCHES "OPTIONAL")
        createOrAppendTo ("OPTIONAL_FEATURES" FEATURE "${APD_FEATURE}" RECORD "${output}"    QUIET EXTEND        TEMPLATE "${template}")
        createOrAppendTO ("OPTIONAL_FEATURES" FEATURE "PACKAGES"       FIELD  ${APD_FEATPKG} QUIET EXTEND UNIQUE TEMPLATE "${template}")
        createOrAppendTO ("OPTIONAL_FEATURES" FEATURE "NAMES"          FIELD  ${APD_FEATURE} QUIET EXTEND UNIQUE TEMPLATE "${template}")
    endif()
...
function(resolveDependencies resolveThese_ featureCollection_ featuresOut_ namesOut_)
    ...
    function(visit lol feature_name package_name is_a_prereq)
        if (NOT "${feature_name}/${package_name}" IN_LIST visited)
            ...
            if (${is_a_prereq})
                list(APPEND resolvedNames "${feature_name}.*/${package_name}")
            else ()
                list(APPEND resolvedNames "${feature_name}/${package_name}")
            endif ()
