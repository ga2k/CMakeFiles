...
macro(fetchContentsHelp)
    ...
    list(APPEND help_msg "USE <ALL | PKG_CATEGORY[:ALT] [PKG_CATEGORY[:ALT] [...]]>")
    list(APPEND help_msg "\t\tFetch_Content() the named package categories")
    list(APPEND help_msg " ")
    list(APPEND help_msg "NOT <PKG_CATEGORY [PKG_CATEGORY [...]]>")
    list(APPEND help_msg "\t\tDon't Fetch_Content() these package categories")
    list(APPEND help_msg " ")
    list(APPEND help_msg "OVERRIDE_FIND_PACKAGE ALL | PKG_CATEGORY [PKG_CATEGORY [...]]")
    list(APPEND help_msg "\t\tRedirect calls to find_package() to this local cache")
    list(APPEND help_msg " ")
    list(APPEND help_msg "FIND_PACKAGE_ARGS [\"ALL [args]\"] [\"PKG_CATEGORY [args]\"] [\"PKG_CATEGORY [args]\"] [...]")
    ...
    list(APPEND help_msg "\t\tIf \"ALL [args]\" supplied, [args] will be prepended to each PKG_CATEGORY's args.")
    list(APPEND help_msg "\t\tDon't add COMPONENTS here, there is a separate keyword for that, shown below.")
    list(APPEND help_msg " ")
    list(APPEND help_msg "FIND_PACKAGE_COMPONENTS \"PKG_CATEGORY args\" [\"PKG_CATEGORY args\" [...]]")
    ...
    list(APPEND help_msg " ")
    list(APPEND help_msg "\tCurrently available package categories are ")
    list(APPEND help_msg " ")

    list(LENGTH PKG_CATEGORIES items)
    math(EXPR last_item "${items} - 1")

    # Find longest package category
    string(LENGTH "Package Category" TITLE_LENGTH)
    set(LONGEST_PKG_LENGTH ${TITLE_LENGTH})

    foreach (x IN LISTS ${PKG_CATEGORIES})
        unset(THS_PKG_CAT)
        getPkgCat("${x}" THIS_PKG_CAT)
        string(LENGTH ${THS_PKG_CAT} THS_PKG_LENGTH)
        if (${THS_PKG_LENGTH} GREATER ${LONGEST_PKG_LENGTH})
            set(LONGEST_PKG_LENGTH ${THS_PKG_LENGTH})
        endif ()
    endforeach ()

    set(NAME "Package Category")
    ...
    foreach (x IN LISTS PKG_CATEGORIES)
        ...
        string(LENGTH ${THIS_PKG_CAT} THS_PKG_LENGTH)
        math(EXPR PADDING "${LONGEST_PKG_LENGTH} - ${THS_PKG_LENGTH} + 3")
        string(REPEAT "." ${PADDING} PAD)
        list(APPEND help_msg "\t${THIS_PKG_CAT} ${PAD} ${THS_PKG_LIST}")
    endforeach ()
    list(APPEND help_msg " ")
...
######################################################################################
##
function(getCatPkgList array cat list_var)
    # iterate over array to find line with cat
    foreach (line IN LISTS array)
        SplitAt("${line}" "|" this_cat packages)
        if ("${this_cat}" STREQUAL "${cat}")
...
######################################################################################
##
function(getCatIndex array cat ix_var)
    # iterate over array to find line with cat
    set(ix -1)
    foreach (line IN LISTS "${array}")
        math(EXPR ix "${ix} + 1")
...
######################################################################################
##
function(getCatPackage array cat index var)
    foreach (item IN LISTS ${array})
        SplitAt("${item}" "|" this_cat packages)
        if ("${this_cat}" STREQUAL "${cat}")
...
######################################################################################
##
function(getPkgCat line var)
    unset(${var} PARENT_SCOPE)
    set(cat "")
    set(dc "")
    SplitAt("${line}" "|" cat unused)
    string(STRIP "${cat}" cat)
    set(${var} "${cat}" PARENT_SCOPE)
endfunction()
##
######################################################################################
##
function(parsePackage pkgArray)
    ...
    if (NOT A_PP_CAT)
        message(FATAL_ERROR "parsePackage() needs CAT parameter")
    endif ()

    if (NOT A_PP_PKG_INDEX AND NOT A_PP_PKG_NAME)
        set(A_PP_PKG_INDEX 0)
    elseif (A_PP_PKG_INDEX AND A_PP_PKG_NAME)
        message(FATAL_ERROR "parsePackage() both PKG_NAME and PKG_CAT supplied. Need one or the other")
    elseif (NOT "${A_PP_PKG_NAME}" STREQUAL "")
        getCatPkgList("${pkgArray}" ${A_PP_CAT} pkg_list)
