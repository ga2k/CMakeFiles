    ...
    if (EXISTS ${WX_OVERRIDE_PATH})
        ...
        foreach(lib ${components})
            ...
            foreach(_variant wx::${lib} wx${lib} ${lib})
                if (TARGET ${_variant})
                    get_target_property(_foundTarget "${_variant}" TYPE)
                    get_target_property(_aliasTarget "${_variant}" ALIASED_TARGET)
                    if (NOT "${_foundTarget}" STREQUAL "_foundTarget-NOTFOUND" AND "${_aliasTarget}" STREQUAL "_aliasTarget-NOTFOUND")
                        target_include_directories(${_variant} SYSTEM BEFORE INTERFACE "${WX_OVERRIDE_PATH}")
                        set (_dirSet ON)
                        break()
                    elseif (NOT "${_foundTarget}" STREQUAL "_foundTarget-NOTFOUND" AND NOT "${_aliasTarget}" STREQUAL "_aliasTarget-NOTFOUND")
                        target_include_directories(${_aliasTarget} SYSTEM BEFORE INTERFACE "${WX_OVERRIDE_PATH}")
                        set (_dirSet ON)
                        break()
                    endif ()
                endif ()
