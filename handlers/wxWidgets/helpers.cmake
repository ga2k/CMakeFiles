...
function(wxWidgets_export_variables pkgname)
    ...
    if (TARGET ${_wx_main_target})
        ...
        msg("get_target_property(_raw_includes ${_wx_main_target} INTERFACE_INCLUDE_DIRECTORIES)")
        get_target_property(_raw_includes ${_wx_main_target} INTERFACE_INCLUDE_DIRECTORIES)
        msg("if (_raw_includes)")
        if (_raw_includes)
            msg("    foreach(_path IN LISTS _raw_includes)")
            foreach (_path IN LISTS _raw_includes)
                msg("        (_path='${_path}'")
                # Strip any generator expressions to get a plain path
                msg("        string(REGEX REPLACE \"\\$<[^>]*>\" \"\" _clean_path \"\${_path}\"")
                string(REGEX REPLACE "\\$<[^>]*>" "" _clean_path "${_path}")
                msg("        (_clean_path='${_clean_path}')")
                msg("        if (_clean_path)")
                if (_clean_path)
                    msg("            list(APPEND local_includes \"\${_clean_path}\")")
                    list(APPEND local_includes "${_clean_path}")
                    msg("            (local_includes='${local_includes}'")
                    msg("        endif()")
                msg("endif ()")
                endif ()
                msg("    endforeach()")
